...
OfferPotato 是一个本地优先的面试准备系统，统一处理三类输入：
...
该路径保留了 `codex-cli` 的原生交互能力，同时由 OfferPotato 提供模型选择、文档引用、文件注入和实时刷新等附加能力。
...
OfferPotato 与 `codex-cli` 的集成分为三条链路。
...
该模式建立在 `codex exec` 之上，但由 OfferPotato 负责上下文治理。除了用户当前消息之外，系统还会注入：
...
- `codex-cli` 负责推理和文件操作
- OfferPotato 负责上下文拼装、任务治理和结果结构化
