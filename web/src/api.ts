...
import { assertDemoRequestAllowed, mapDemoApiInput } from './demoApiRouter'
...
async function request<T>(input: string, init?: RequestInit): Promise<T> {
  ...
  if (demoMode) {
    assertDemoRequestAllowed(method)
  }

  const resolvedInput = demoMode ? mapDemoApiInput(input) : input
